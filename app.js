...
(function () {
  ...
  function showContactDetail(id) {
    ...
    const infoItems = [
      { label: '電話番号', value: contact.phone, icon: 'phone', link: contact.phone ? `tel:${contact.phone}` : null },
      { label: '携帯番号', value: contact.mobile, icon: 'phone', link: contact.mobile ? `tel:${contact.mobile}` : null },
      { label: 'FAX番号', value: contact.fax, icon: 'printer' },
...
読み取れない項目は空文字列 "" にしてください。フリガナがない場合は名前から推測してカタカナで入れてください。`
  ...
  function getFormData() {
    return {
      ...
      fax: $('#field-fax') ? $('#field-fax').value.trim() : '',
